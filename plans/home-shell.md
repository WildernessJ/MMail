...
- [x] **T005 (SC: 004, 005): Implement InboxGlance.project + commit** — Filter `emails` to `folder == "inbox"` and (account=="all" ? all : `email.account == account`) and `unread`; `unread` = count; `newToday` = count where `sortDate.map { Calendar.current.isDate($0, inSameDayAs: now) } == true`; `peek` = sorted by `AppModel.isNewerFirst` then `prefix(5)` as `Array`.
...
- [x] **T008 (SC: 001): Settings → Home section** — Add a "Home" group/section in `SettingsView` containing six `MMToggle` rows (Date, Weather, Inbox glance, People, Journal, To-do), each bound via `Binding(get: { model.homeWidgets[w] }, set: { model.setHomeWidget(w, on: $0) })`. Match the existing section styling (see the Appearance `Picker` block).
...
- [x] **T012 (SC: 001–008): Full suite + type-check green** — Run the entire test suite from the MAIN checkout and the type-check build; ensure no regression in the existing suites.
